...
class TextRecognizer:
    ...
    def __init__(self, models_dir: Optional[Union[str, Path]] = None) -> None:
        ...
        self.models_dir = (
            Path(models_dir)
            if models_dir
            else Path(__file__).parent / "paddleocr_models"
        )
        ...
        self.model = PaddleOCR(
            ...
            det_model_dir="src/models/paddleocr_models/det",
            rec_model_dir="src/models/paddleocr_models/rec",
        )
    ...
    def _process_multiple_tables(
        self, ocr_data: List, table_boxes: np.ndarray
    ) -> List[pd.DataFrame]:
        ...
        for item in ocr_data:
            ...
            for idx, table_box in enumerate(table_boxes):
                if (
                    bbox[0] >= table_box[0]
                    and bbox[1] >= table_box[1]
                    and bbox[0] <= table_box[2]
                    and bbox[1] <= table_box[3]
                ):
